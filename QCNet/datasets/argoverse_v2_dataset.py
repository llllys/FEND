...
class ArgoverseV2Dataset(Dataset):
    ...
    def __init__(self,
        ...
        if processed_dir is None:
            ...
            # processed_dir = os.path.join(root, split, 'processed_score')
        ...
        # offline cluster label
        cluster_path = '###YOUR_DIRECTORY###/scene_cluster_dict.pt'
        self.cluster_label = torch.load(cluster_path, weights_only=True)
        
        # offline difficulty score
        # score_path = '~/FEND/QCNet/results/train_prediction_0326.pt'
        # self.score = torch.load(score_path, weights_only=True)
    ...
    def get(self, idx: int) -> HeteroData:
        ...
        if 'train' in self.processed_paths[idx]:
            data['agent']['cluster'] = self.cluster_label[data['scenario_id']]
            
            # offline difficulty score
            # data['agent']['score'] = self.score[data['scenario_id']]['score']
