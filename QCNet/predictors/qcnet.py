...
# Modified by Yuansheng Lian on March 2025:
# - Incorperate FEND in training
...
class QCNet(pl.LightningModule):
    ...
    def __init__(self,
                 ...
                 validation_results_dir: str = './results',
                 val_prediction_name: str = 'val_prediction',
        ...
        self.val_prediction_name = val_prediction_name
        ...
        print('Contrastive learning: ', self.contrastive_learning)
        if self.contrastive_learning:
            self.difficulty_threshold = 0.5 # ade
            self.contrastive_warmup_epochs = 1
            self.x_a_emb = MLPLayer(input_dim=hidden_dim, hidden_dim=4 * hidden_dim, output_dim=hidden_dim)
            self.pcl_loss = PCLLoss()
    ...
    def forward(self, data: HeteroData):
        ...
        if self.contrastive_learning and self.training:
            x_a_emb = self.x_a_emb(scene_enc['x_a'][:, -1, :]) # [n, d]
            pred['x_a_emb'] = x_a_emb
    ...
    def training_step(self,
        ...
        # loss by agent
        reg_loss_refine_agent = reg_loss_refine.sum(dim=1)  # Shape: [B]
        valid_counts = reg_mask.sum(dim=1).clamp(min=1)  # Shape: [B], number of valid timesteps per agent
        reg_loss_refine_agent = reg_loss_refine_agent / valid_counts  # Shape: [B]
        
        reg_loss_refine = reg_loss_refine.sum(dim=0) / reg_mask.sum(dim=0).clamp_(min=1) # Shape: [T]
        ...
        if self.contrastive_learning and self.current_epoch >= self.contrastive_warmup_epochs:
            pcl_loss_agent = self.pcl_loss(pred['x_a_emb'], data['agent']['cluster'])
            
            hardness_mask = reg_loss_refine_agent > self.difficulty_threshold
            pcl_loss_agent = pcl_loss_agent * hardness_mask.float()
            pcl_loss = pcl_loss_agent.mean()
        
            loss += pcl_loss
            self.log('train_pcl_loss', pcl_loss, prog_bar=False, on_step=True, on_epoch=True, batch_size=1)
    ...
    def validation_step(self,
        ...
        # save ade scores
        # ade_score = torch.norm(traj_refine_best[..., 0:2] - gt[..., 0:2], p=2, dim=-1) * reg_mask
        # ade_score = ade_score.sum(dim=1) / reg_mask.sum(dim=1).clamp_(min=1)
        # if self.dataset == 'argoverse_v2':
        #     for i in range(data.num_graphs):
        #         scenario_id = data['scenario_id'][i]
        #         start_ind, end_ind = data['agent']['ptr'][i], data['agent']['ptr'][i+1]
        #         self.val_predictions[scenario_id] = {
        #             'score': ade_score[start_ind:end_ind].cpu(),
        #             } 
        # else:
        #     raise ValueError('{} is not a valid dataset'.format(self.dataset))
    ...
    #     if self.dataset == 'argoverse_v2':
    #         save_path = Path(self.validation_results_dir) / f'{self.val_prediction_name}.pt'
    #         print('Save validation results to', save_path)
    #         torch.save(self.val_predictions, save_path)
    #     else:
    #         raise ValueError('{} is not a valid dataset'.format(self.dataset))
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--num_agent_layers', type=int, default=1) # 2
        parser.add_argument('--num_dec_layers', type=int, default=1) # 2
        parser.add_argument('--num_heads', type=int, default=8)
        parser.add_argument('--head_dim', type=int, default=16)
        parser.add_argument('--dropout', type=float, default=0.1)
        parser.add_argument('--pl2pl_radius', type=float, default=50) # 150
        ...
        parser.add_argument('--pl2m_radius', type=float, default=50) # 150
        parser.add_argument('--a2m_radius', type=float, default=50) # 150
        parser.add_argument('--lr', type=float, default=4e-5)
        parser.add_argument('--weight_decay', type=float, default=1e-4)
        parser.add_argument('--T_max', type=int, default=64)
        parser.add_argument('--submission_dir', type=str, default='./')
        parser.add_argument('--validation_results_dir', type=str, default='./results')
        parser.add_argument('--submission_file_name', type=str, default='submission')
        parser.add_argument('--contrastive_learning', type=bool, default=False)
        parser.add_argument('--val_prediction_name', type=str, default='val_prediction')
